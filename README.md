...
`Crazy Ivan` is going to lend itself at being great at checking anything it can from a command line, and at this point cannot mock user input into a web application, for example.

## Credits and License

Developed by Greg Gallagher (frellus), Copyright 2023 

Note: I credit my former collegue Joel, who originally came up with the name "Marvin", for the first framework of checks (written in Perl). I then moved into a different job (the one with Ivan, the story above) and conceptually developed the same solution from scratch. At some point I re-write it in Python, but it was critical ifrastructure used to run thousands and thousands of checks globally. Still, I chose to name it `marvin` as a tribute to Joel, even it ended up being a *much* larger scope than the first one.

Crazy Ivan is the first time I am creating a `marvin` for public consumption. Other than the overall generic concept, this is a completely original work and fresh approach, as I think the problem is wide ranging, generic and timeless. None of the enterprise monitoring systems we used *in the day* survived to be still in use, and the cloud and SaaS monitoring solutions didn't exist at that time either -- but I hear some whispers that `marvin` is still in use at that place I left 10 years ago.accordion
